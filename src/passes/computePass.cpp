...
void ComputePass::createDescriptorSetLayout(VkDevice device) {
    ...
    if (vkCreateDescriptorSetLayout(device, &ci, nullptr, &descriptorSetLayout_) != VK_SUCCESS)
        throw std::runtime_error("ComputePass: failed to create descriptor set layout.");
}
...
void ComputePass::createDescriptorPool(VkDevice device) {
    ...
    sizes[0].descriptorCount = 2 * kMaxFramesInFlight; // node + material per frame
...
void ComputePass::allocateDescriptorSets(VkDevice device) {
    ...
    if (vkAllocateDescriptorSets(device, &ai, descriptorSets_.data()) != VK_SUCCESS)
        throw std::runtime_error("ComputePass: failed to allocate descriptor sets.");
}
...
void ComputePass::createPipelineLayout(VkDevice device) {
    ...
    if (vkCreatePipelineLayout(device, &ci, nullptr, &pipelineLayout_) != VK_SUCCESS)
...
void ComputePass::writeDescriptors(VkDevice device,
                                    VkBuffer nodeBuffer,     VkDeviceSize nodeBytes,
...
void ComputePass::updateStorageImage(VkDevice device, VkImageView storageImageView) {
    VkDescriptorImageInfo imgInfo{};
    imgInfo.imageView   = storageImageView;
    imgInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;
...
void ComputePass::rebindBuffers(VkDevice device,
                                 VkBuffer nodeBuffer,     VkDeviceSize nodeBytes,
                                 VkBuffer materialBuffer, VkDeviceSize materialBytes)
{
    for (int f = 0; f < kMaxFramesInFlight; ++f) {
...
VkShaderModule ComputePass::createShaderModule(VkDevice device, const std::vector<char>& code) {
...
std::vector<char> ComputePass::readSpv(const std::string& path) {
    ...
    if (!file.is_open())
        throw std::runtime_error("ComputePass: failed to open SPIR-V file: " + path);
